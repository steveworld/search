...
/**
 * Factory class for registering and searching for expressions for use in
 * the {@link org.apache.hadoop.fs.shell.find.Find} command.
 */
public final class ExpressionFactory {
  ...
  /**
   * Register the given class as handling the given list of expression
   * names.
   * @param expressionClass the class implementing the expression names
   * @param names one or more command names that will invoke this class
   * @throws IOException if the expression is not of an expected type
   */
  public void addClass(Class<? extends Expression> expressionClass, String ... names) throws IOException{
    for (String name : names) expressionMap.put(name, expressionClass);
  }
  public boolean isExpression(String expressionName) {
    return expressionMap.containsKey(expressionName);
  }
  /**
   * Get an instance of the requested expression
   * @param expressionName name of the command to lookup
   * @param conf the hadoop configuration
   * @return the {@link Expression} or null if the expression is unknown
   */
  public Expression getExpression(String expressionName, Configuration conf) {
    if (conf == null) throw new NullPointerException("configuration is null");
    
    Class<? extends Expression> expressionClass = expressionMap.get(expressionName);
    Expression instance = createExpression(expressionClass, conf);
    return instance;
  }
  
  public Expression createExpression(Class<? extends Expression> expressionClass, Configuration conf) {
  ...
  public Expression createExpression(String expressionClassname, Configuration conf) {
    try {
