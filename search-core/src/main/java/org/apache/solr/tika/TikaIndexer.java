...
public class TikaIndexer extends SolrIndexer {
  ...
  public TikaIndexer(SolrCollection solrCollection, Config config, MetricRegistry metricRegistry) {
    super(solrCollection, config, metricRegistry);
...
//  DefaultParser tikaConfigParser = new DefaultParser(getTikaConfig().getMediaTypeRegistry());
  ...
  public void process(StreamEvent event) throws IOException, SolrServerException, SAXException, TikaException {
    parseInfo = new ParseInfo(event, this, mediaTypeToParserMap, getMetricRegistry()); // ParseInfo is more practical than ParseContext
    try {
      super.process(event);
