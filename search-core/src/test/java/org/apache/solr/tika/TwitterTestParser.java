...
import org.apache.solr.tika.parser.AbstractStreamingParser;
 ...
 */
public class TwitterTestParser extends ReentrantParser {

  private static final MediaType MEDIA_TYPE = MediaType.parse("mytwittertest/json+delimited+length");
  
  @Override
  protected StreamingTwitterTestParser createInstance() {
    StreamingTwitterTestParser parser = new StreamingTwitterTestParser();
    parser.setSupportedTypes(Collections.singleton(MEDIA_TYPE));
    return parser;
  }

  
  private static final class StreamingTwitterTestParser extends AbstractStreamingParser {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(StreamingTwitterTestParser.class);
  
    public StreamingTwitterTestParser() {
    }
    
    @Override
    protected void doParse(InputStream in, ContentHandler handler) throws IOException, SolrServerException {
  
      getParseInfo().setMultiDocumentParser(true);
      ...
      try {
        while (true) {
          ...
          load(Collections.singletonList(doc));
          numRecords++;
        }
      } finally {
